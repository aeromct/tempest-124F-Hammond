...
C
C
c     SUBROUTINE GGM(ART,LONG,LATI,MLONG,MLAT)            
...
c     INTEGER ART     
c     REAL MLONG,MLAT,LONG,LATI
c     COMMON/CONST/FAKTOR
c     ZPI=FAKTOR*360.                              
c     CBG=11.4*FAKTOR                              
c     CI=COS(CBG)     
c     SI=SIN(CBG)
c     IF(ART.EQ.0) GOTO 10                         
c     CBM=COS(MLAT*FAKTOR)                           
c     SBM=SIN(MLAT*FAKTOR)                           
c     CLM=COS(MLONG*FAKTOR)                          
c     SLM=SIN(MLONG*FAKTOR)
c     SBG=SBM*CI-CBM*CLM*SI
c       IF(ABS(SBG).GT.1.) SBG=SIGN(1.,SBG)
c     LATI=ASIN(SBG)
c     CBG=COS(LATI)     
c     SLG=(CBM*SLM)/CBG  
c     CLG=(SBM*SI+CBM*CLM*CI)/CBG
c       IF(ABS(CLG).GT.1.) CLG=SIGN(1.,CLG)                  
c     LONG=ACOS(CLG)  
c     IF(SLG.LT.0.0) LONG=ZPI-LONG
c     LATI=LATI/FAKTOR    
c     LONG=LONG/FAKTOR  
c     LONG=LONG-69.8    
c     IF(LONG.LT.0.0) LONG=LONG+360.0                 
c     RETURN          
c10   YLG=LONG+69.8    
c     CBG=COS(LATI*FAKTOR)                           
c     SBG=SIN(LATI*FAKTOR)                           
c     CLG=COS(YLG*FAKTOR)                          
c     SLG=SIN(YLG*FAKTOR)                          
c     SBM=SBG*CI+CBG*CLG*SI                        
c       IF(ABS(SBM).GT.1.) SBM=SIGN(1.,SBM)
c     MLAT=ASIN(SBM)   
c     CBM=COS(MLAT)     
c     SLM=(CBG*SLG)/CBM                            
c     CLM=(-SBG*SI+CBG*CLG*CI)/CBM
c       IF(ABS(CLM).GT.1.) CLM=SIGN(1.,CLM) 
c     MLONG=ACOS(CLM)
c     IF(SLM.LT..0) MLONG=ZPI-MLONG
c     MLAT=MLAT/FAKTOR    
c     MLONG=MLONG/FAKTOR  
c     RETURN          
c     END             
c
