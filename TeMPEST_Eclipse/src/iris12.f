...
C
C
      SUBROUTINE IRIS12(path,JF,JMAG,ALATI,ALONG,RZ12,MMDD,DHOUR,
...
c     if(icalls.gt.1) goto 8201
c	write(*,*) '*** IRI parameters are being calculated ***'
c     if(NODEN) goto 2889
...
c	if(OLD79) write(*,*) 'Ne: Using IRI-79. Correction',
c    &	  ' of equatorial topside is not included.'
...
c	endif
c2889 if((.not.NOION).and.(DY)) 
c    &	   write(*,*) 'Ion Com.: Using Danilov-Yaichnikov-1985.'
c     if((.not.NOTEM).and.(TENEOP))
c    &     write(*,*) 'Te: Temperature-density correlation is used.'
8201	continue
C
...
C
7797    WRITE(FILNAM,104) path, MONTH+10
        URSIFO=URSIF2
1344    lread=1
...
C!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
C!! FOR ASCII CODED CCIR COEFFIECENTS FILES SUBSTITUTE  !!
C!! THE LAST THREE STATEMENTS WITH THE FOLLOWING FOUR:  !!
C!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
C
	if(URSIF2) then
	  WRITE(FILNAM,1144) path, MONTH+10
1244      lread=2
...
C          READ(IUCCIR) F2
C!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
C!! FOR ASCII CODED URSI COEFFIECENTS FILES SUBSTITUTE  !!
...
8449	FORMAT(1X////,
     &	  ' !!!! The file ',A,' is not in your directory,'/
     &	  ' !!!!   try a different diskette (enter: 1),'/
     &    ' !!!!   or exit (enter: 0)')
	read(*,*) idisk
	if(idisk.eq.1) goto (1344,1244) lread
	GOTO 3330
C
