...
C
C***********************************************************************
      SUBROUTINE GTS5(path,IYD,SEC,ALT,GLAT,GLONG,STL,
     +                F107A,F107,AP,MASS,D,T)
      character*(*) path
...
C       PARMS NOT USED: 13
   ...
   50 TT=TT+ABS(SW(I))*T(I)
      ...
      END
C--------------------------------------------------------------------
C     FUNCTION DNET(DD,DM,ZHM,XMM,XM)
C       8/20/80
...
C       Eq. A12a
C     YLOG=A*ALOG(DM/DD)
C     IF(YLOG.LT.-10.) GO TO 10
C     IF(YLOG.GT.10.)  GO TO 20
C       DNET=DD*(1.+EXP(YLOG))**(1/A)
C       GO TO 50
C  10 CONTINUE
C       DNET=DD
C       GO TO 50
...
C     END
C--------------------------------------------------------------------
C     FUNCTION  CCOR(ALT, R,H1,ZH)
C        CHEMISTRY/DISSOCIATION CORRECTION FOR MSIS MODELS
C     Eq. A20a or Eq. A21
C     E=(ALT-ZH)/H1
C     IF(E.GT.70.) GO TO 20
C     IF(E.LT.-70.) GO TO 10
C       EX=EXP(E)
C       CCOR=R/(1.+EX)
C       GO TO 50
C  10   CCOR=R
...
C          CIRA     11-FEB-86   
     ...
     * PH1(50),PH2(50),PH3(50),PI1(50)
      ...
      write (fname, 11111) path
11111 format (A,'MSIS86.DAT')

2392  OPEN(UNIT=IUMSIS,FILE=fname,FORM='FORMATTED',
     &	  ERR=2399,STATUS='OLD')
...
1290  FORMAT(1X,5E13.6)
      GOTO 2395
2399  WRITE(*,2396) fname
2396  FORMAT(' THE MSIS COEFFICIENT FILE (',A,') IS NOT IN',
     &  ' YOUR DIRECTORY.')
      IIEE=1
     	RETURN
2390  WRITE(*,2397) fname
2397  FORMAT(' ERROR IN READING THE MSIS COEFFICIENT FILE (',A,')')
      IIEE=1
	RETURN
2395  DO 10 I=1,3
        ISDATE(I)=ISD(I)
